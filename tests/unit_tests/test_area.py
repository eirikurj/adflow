...
class AreaCalcTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_fwd(self):
        ...
        # res not accurate because of if-statement branching caused by perturbations
        ...
        np.testing.assert_allclose(hfDot_FD[hfDot_FD != 0], hfDot[hfDot_FD != 0], rtol=2e-5)
    ...
    @unittest.skip("numerical error issues")
    def test_bwd(self):
        ...
        import pickle
        with open("resDot_CS.p", "rb") as fid:
            resDot_CS = pickle.load(fid)
        
        err = (resDot_CS - resDot)/(resDot)
        idx_max_err = np.argmax(np.abs(err))
        # np.testing.assert_allclose(resDot_CS,resDot, atol=1e-14)
        # import pdb; pdb.set_trace()
        

        dwBar = self.CFDSolver.getStatePerturbation(314)
        xVBar = self.CFDSolver.computeJacobianVectorProductBwd(resBar=dwBar, xVDeriv=True)
        # import pdb; pdb.set_trace()
