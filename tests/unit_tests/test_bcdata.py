...
class BCGetSetTests(unittest.TestCase):
    def setUp(self):
        ...
        self.options = {"gridfile": gridFile, "equationType": "Euler"}

        # Aerodynamic problem description (taken from test 17)
        ap = AeroProblem(name="nozzle", alpha=90, mach=0.5, altitude=0, areaRef=1.0, chordRef=1.0, R=287.87)
        ...
        self.initial_dvs = {
            ...
            "upstream": {
                ...
                "VelocityUnitVectorX": 0.0,
                "VelocityUnitVectorY": 1.0,
    ...
    def test_setBCData(self):
        ...
        for patch in new_bc_data:
            for arr in patch:
                np.testing.assert_array_equal(arr.data, np.array(self.set_dvs[patch.fam][arr.name]), err_msg=arr.name)
    ...
    def test_getBCData_array(self):
        ...
        for patch in bc_data:
            for arr in patch:
                np.testing.assert_array_equal(arr.data, np.ones(arr.size) * self.initial_dvs[patch.fam][arr.name])
    ...
    def test_setBCData_array(self):
        ...
        for patch in bc_data:
            for arr in patch:
                np.testing.assert_array_equal(arr.data, np.ones(arr.size) * self.set_dvs[patch.fam][arr.name])
...
class BCDerivsTests(unittest.TestCase):
    ...
    def test_fwd_array_vs_float(self):
        ...
        resDot_dict, funcsDot_dict, fDot_dict, hfDot_dict = self.CFDSolver.computeJacobianVectorProductFwd(
            xDvDot={"outlet_pressure": np.ones(5)},
            residualDeriv=True,
            funcDeriv=True,
            fDeriv=True,
            hfDeriv=True,
...
class BCNodalDerivsTests_serial(unittest.TestCase):
    ...
    def test_fwd_vs_float(self):
        ...
        resDot_dict, funcsDot_dict, fDot_dict, hfDot_dict = self.CFDSolver.computeJacobianVectorProductFwd(
            xDvDot={"outlet_pressure": np.ones(4 * 9 + 1)},
            residualDeriv=True,
            funcDeriv=True,
            fDeriv=True,
            hfDeriv=True,
...
class BCNodalDerivsTests_parallel(unittest.TestCase):
    ...
    def test_fwd_vs_float(self):
        ...
        resDot_dict, funcsDot_dict, fDot_dict, hfDot_dict = self.CFDSolver.computeJacobianVectorProductFwd(
            xDvDot={"outlet_pressure": np.ones(n)},
            residualDeriv=True,
            funcDeriv=True,
            fDeriv=True,
            hfDeriv=True,
