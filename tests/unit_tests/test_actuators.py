...
class ActuatorBasicTests(unittest.TestCase):
    def setUp(self):
        ...
        self.CFDSolver.addActuatorRegion(
            actuatorFile, np.array([0, 0, 0]), np.array([1, 0, 0]), "actuator", thrust=self.force
        )
    ...
    def test_mom_flux(self):
        ...
        force_cfd = (
            -funcs[self.ap.name + "_mdot_out"] * funcs[self.ap.name + "_vx_out"]
            + funcs[self.ap.name + "_ps_out"] * funcs[self.ap.name + "_area_out"]
            - (
                funcs[self.ap.name + "_mdot_in"] * funcs[self.ap.name + "_vx_in"]
                + funcs[self.ap.name + "_ps_in"] * funcs[self.ap.name + "_area_in"]
            )
        )
